...
if uploaded_file is not None:
    try:
        ...
        # 1. CHUẨN HÓA THỜI GIAN (Giữ nguyên logic của bạn)
        ...
        # Giữ nguyên toàn bộ logic lọc if/elif của bạn để đảm bảo sel_label chuẩn
        if view_mode == "Ngày":
        ...
        # 5. HIỂN THỊ SỐ LIỆU (Đã gộp - Chỉ hiện 1 lần duy nhất)
        if not filtered_df.empty:
            ...
            with c3:
                if 'humiKK' in filtered_df.columns: st.metric("Độ ẩm KK", f"{filtered_df['humiKK'].mean():.1f} %")
                elif 'Độ ẩm' in filtered_df.columns: st.metric("Độ ẩm Đất", f"{filtered_df['Độ ẩm'].mean():.1f} %")
            with c4:
            ...
            # --- HÀNG 2 (NPK, AS, TDS...) ---
            extra_metrics = {'AS': 'Áp suất', 'TDS EC': 'TDS', 'N': 'Nitơ', 'P': 'Photpho', 'K': 'Kali', 'Lưu lượng m2/h': 'Lưu lượng tức thời'}
            available_extras = [col for col in extra_metrics.keys() if col in filtered_df.columns]
            
            if available_extras:
                st.markdown("---")
                cols = st.columns(4)
                for idx, col in enumerate(available_extras):
                    val = filtered_df[col].mean()
                    if pd.notnull(val): cols[idx % 4].metric(extra_metrics[col], f"{val:.2f}")

            # 6. BIỂU ĐỒ (Giữ nguyên logic gốc của bạn)
            st.subheader("📈 Biểu đồ diễn biến")
            numeric_cols = filtered_df.select_dtypes(include=['number']).columns.tolist()
            chart_metrics = [m for m in numeric_cols if m not in ['Lưu lượng tổng']]
            
            selected_m = st.multiselect("Thêm thông số:", chart_metrics, default=[chart_metrics[0]] if chart_metrics else [])
            
            if selected_m:
                # Đây là đoạn logic vẽ hình "chuẩn" của bạn:
                if view_mode == "Ngày":
            ...
            with st.expander("🔍 Chi tiết bảng"):
